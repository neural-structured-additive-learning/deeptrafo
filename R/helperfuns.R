...
comp_cdf <- function(x, y) {
  ...
  for (k in 0L:(number_points - 1L)) {
    
    yy <- tf$reshape(tf$gather(y, 0L:k), list(k + 1L))
  ...
  tf$assert_greater(integrals, -1e-3)
...
lin_interpol <- function(p_val, x, y) {
  ...
  tf$assert_equal(x$shape[1], y$shape)
  tf$assert_less(x, 1 + 1e-3)
  tf$assert_greater(x, 0 - 1e-3)
  
  if (tf$raw_ops$UniqueWithCountsV2(x = x, axis = list(0L))$y$shape[1] == 1) {
    x <- tf$add(x, tf$linspace(1e-5, 1e-4, x$shape[1]))
  }
  
  F_y_hat_diff <- tf$experimental$numpy$diff(tf$reshape(x, list(as.integer(x$shape[1]))))
  y_grid_diff <- tf$experimental$numpy$diff(y)
  
  tf$assert_greater(F_y_hat_diff, -1e-3) # monotone (but not strictly) increasing
  tf$assert_greater(y_grid_diff, -1e-3)
  
  relevant_interval <- tf$searchsorted(tf$reshape(x, list(as.integer(x$shape[1]))), p_val, side = "right")
  ...
  x <- tf$reshape(x, list(as.integer(x$shape[1])))
